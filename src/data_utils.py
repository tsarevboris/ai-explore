...
def _unzip(zip_path: Path, extract_to: Path) -> List[Path]:
    ...
    return [extract_to / name for name in zf.namelist()]  # paths to extracted files

def _run(cmd: list[str]) -> None:
    # Runs a shell command, raises on error
...
def download_kaggle_dataset(
    ...
    if force or not zip_path.exists():
        # kaggle datasets download -d camnugent/california-housing-prices -p data/raw
